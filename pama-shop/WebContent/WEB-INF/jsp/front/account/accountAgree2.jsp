...
<title>검진</title>
<script type="text/javascript" src="/common/js/jquery-1.6.2.min.js" charset="utf-8"></script>
...
<!--
	function isForms(s) {
		...
		var birth4 = form.birth1.value +  form.birth2.value +  form.birth3.value;
		form.birth.value=birth4;

		var email4 = form.email1.value + form.email2.value;
		form.email.value=email4;
		form.action = "/mbr/accountJoinProc.do";
		form.submit();
		alert("dd");
		return true;
	}
	...
	function getMoblieChk() {
		...
		if (retVal != "") {
			...
			alert("핸드폰번호 첫문자는 '_'로 시작할수 없습니다.");
			f.mobile.focus();
		} else if (f.mobile.value.length < 11) {
			alert("핸드폰번호 11자 이상입니다.");
			f.mobile.focus();
		} else if (chkNonchar(f.mobile.value)) {
			alert("핸드폰번호 특수문자는 입력할 수 없습니다.");
			f.mobile.focus();
		} else if (!isID.test(str)) {
			alert("핸드폰번호 5~20자의 영문 소문자와 숫자,특수기호(_)만 사용할 수 있습니다.");
			f.mobile.focus();
		} else {
			parm1 = f.mobile.value;
			var statusTypeChangeUrl = '/signup/accountMobilechk.do?mobile='
					+ parm1;
			$.getJSON(statusTypeChangeUrl, function(data) {
				if (data.mobileChk != null) {
					alert("이미 사용중인 핸드폰번호 입니다. 다른 핸드폰번호를 입력하세요.");
					f.mobilechkYn.value = "";
				} else {
					alert("사용 가능한 핸드폰번호 입니다.");
					f.mobilechkYn.value = "Y";
				}
			});
			return true;
		}
	...
	function checkReset(type) {
		...
		var select = document.accountForm.email3;
		if (type == 'id') {
			form.idchkYn.value = "";
		} else if (type == 'email') {
			...
			(type == 'email4')
		{
			form.email2.value = select.options[select.selectedIndex].value;
		}
	}
//-->
</script>
				...
				<div class="bbs">
					...
					<div class="join_fied">
						<fieldset>
							...
							<input type="hidden" name="idchkYn" id="idchkYn" class="idchkYn" value="${not empty accountFormVO?'Y':''}" class="required" /> <input type="hidden" name="mobilechkYn" id="mobilechkYn" class="mobilechkYn" value="${not empty accountFormVO?'Y':''}" class="required" /> <input type="hidden" name="emailchkYn" id="emailchkYn" class="emailchkYn" value="${not empty accountFormVO?'Y':''}" class="required" /> <input type="hidden" name="birth" id="birth" value="" />
							<table width="100%" summary="" cellpadding="0" cellspacing="0">
								...
								<tbody>
									...
									<tr>
										...
										<td><input type="text" id="firstName" name="firstName" class="inpText" onChange="checkReset('id')" class="required" /></td>
									</tr>
									<tr>
										<th scope="row"><label for="comp_num">생년월일</label> <span class="warning">*</span></th>
										<td><select id="birth1" name="birth1" title="월">
										...
										</select> <select name="birth2" id="birth2" title="월">
										...
										</select> <select name="birth3" id="birth3" title="일">
												<option value="02">1</option>
												<option value="02">2</option>
												<option value="02">3</option>
												<option value="02">4</option>
									...
									<tr>
										...
										<td><input type="text" id="mbrId" name="mbrId" placeholder="회원아이디" title="회원아이디" class="inpText" onChange="checkReset('id');" class="required" /> <a class="leftmag" onClick="getIdChk();"><img src="/join_form_img/sub/bt_overlap.png" align="absmiddle" alt="중복확인" /></a></td>
									</tr>
									<tr>
									<tr>
										<th scope="row"><label for="comp_id">휴대폰번호인증</label> <span class="warning">*</span></th>
										<td><input type="text" id="mobile" name="mobile" class="inpText" placeholder="(-없이)입력" onChange="checkReset('mobile')" class="required" /> <a class="leftmag" onClick="getMoblieChk();"><img src="/join_form_img/sub/bt_overlap.png" align="absmiddle" alt="인증번호발송" /></a></td>
									</tr>
									<tr>
									<tr>
										<th scope="row"><label for="comp_id">인증번호입력</label> <span class="warning">*</span></th>
										<td><input type="text" name="certification" id="certification" placeholder="인증번호입력" class="inpText" /> <a href="#" class="leftmag"><img src="/join_form_img/sub/bt_overlap.png" align="absmiddle" alt="인증하기" /></a></td>
									</tr>
									<tr>
									...
									<tr class="email_bx">
										...
										<td><input type="text" id="email1" name="email1" title="이메일 아이디를 작성하세요." /> @ <input type="text" id="email2" name="email2" title="이메일 계정을 작성하세요." /> <input type="hidden" id="email" name="email" value="" /> <select title="이메일 계정을 선택하세요." id="email3" name="email3" onChange="checkReset('email4')">
									...
									<tr class="adress_bx">
										...
										<td>
											<p>차량 정보를 정확히 입력하면 정확한 견적을 받을 수 있습니다.</p> <select id="brand" name="brand" title="선택하세요">
												<option value="">제조사 선택</option>
												<option value="">현대자동차</option>
												<option value="">기아자동차</option>
												<option value="">르노삼성자동차</option>
												<option value="">쉐보레</option>
										</select> <br> <select id="model" name="model" title="선택하세요">
												<option value="">모델명 선택</option>
												<option value="">포르쉐</option>
												<option value="">르망</option>

										</select> <br> <select id="caryear" name="caryear" title="선택하세요">
												...
												<option value="">2014</option>
												<option value="">2014</option>
										</select> <br> <select id="mission" name="mission" title="선택하세요">
										...
										</select> <br> <input type="text" id="carIdentNumber" name="carIdentNumber" class="inpText" placeholder="차대번호입력" /> <br> <input type="text" id="carNumber" name="carNumber" placeholder="차량번호입력" /> <br> <input type="text" id="distance" name="distance" class="inpText" placeholder="주행거리입력" />Km
										</td>
									</tr>
					...
					<div class="agree_wrap">
						...
						<div class="bt">
							<a href="#;"><img src="/join_form_img/sub/bt_ok.png" alt="확인" onclick="javascript:document.accountForm.submit();"/></a>
							<a href="/main/main.do"><img src="/join_form_img/sub/bt_no.png" alt="취소" /></a>
						</div>
					</div>
				</div>
			</div>
