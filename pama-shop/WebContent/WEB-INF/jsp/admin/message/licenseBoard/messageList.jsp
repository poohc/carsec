...
function issueRegister(portletId, messageId) {
	var targetUrl = '/${boardVO.boardKey}/messageDetail.do?messageId=' + messageId;
	$.ajax({
		url: '/admin/portlet/${boardVO.boardKey}/portletMessageRegister.json',
		data: 'portletId=' + portletId + '&messageId=' + messageId + '&targetUrl=' + targetUrl,
		type: 'POST',
		error: function(){
			alert("오류가 발생하였 습니다. 관리자에게 문의하세요.");
		},
		success: function(json){
			if (json.msgCode == "100") alert("성공적으로 등록 되었습니다.");
			if (json.msgCode == "201") alert("이미 등록 되었습니다.");
		}
	});
    ...
    <tbody>
        <c:forEach items="${messageList.messageList}" var="i" varStatus="iStatus">
            <tr>
                ...
                <td align="center">${i.subTitle} ${fn:length(i.messageCategoryName)>0?'/':'' } ${i.messageCategoryName }</td>
                <td><a href="javascript:goMessageForm('${i.messageId}');">${i.title}</a></td>
                <td align="center">${fn:substring(i.registerDate, 0, 10)}</td>
                <td align="center">${i.licenseType}</td>
